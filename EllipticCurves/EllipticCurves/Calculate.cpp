...
{
	return ((y * y - x * x * x - curveA * x - curveB) % fieldSize == 0);
}

int64_t tryDoublePoint(int64_t &_changedPointX, int64_t &_changedPointY)
{
	int64_t denominator = 2 * _changedPointY;
	denominator %= fieldSize;
	int64_t gcd = greatestCommonDivisor(fieldSize, denominator);
	...
	else
	{
		...
		int64_t changedPointX = (lambda * lambda - 2 * _changedPointX) % fieldSize;
		int64_t changedPointY = ((_changedPointX - changedPointX) * lambda - _changedPointY) % fieldSize;
...
int64_t tryAddPoint(int64_t &_changedPointX, int64_t &_changedPointY, const int64_t _pointToAddX, const int64_t _pointToAddY)
{
	...
	{
		int64_t delta = (_changedPointY - _pointToAddY) % fieldSize;
		const int64_t deltaY = (delta < 0 ? delta + fieldSize : delta);
		delta = (_changedPointX - _pointToAddX) % fieldSize;
		const int64_t deltaX = (delta < 0 ? delta + fieldSize : delta);
		...
		const int64_t gcd = greatestCommonDivisor(fieldSize, deltaX);
		if (gcd > 1)
		{
			return gcd;
		}
		else 
		{
			...
			int64_t changedPointX = (lambda * lambda - _changedPointX - _pointToAddX) % fieldSize;
			int64_t changedPointY = (lambda * (_changedPointX - changedPointX) - _changedPointY) % fieldSize;
			_changedPointX = (changedPointX < 0 ? changedPointX + fieldSize : changedPointX);
			_changedPointY = (changedPointY < 0 ? changedPointY + fieldSize : changedPointY);
